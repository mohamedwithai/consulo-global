    ...
    <>
      <section className="pt-32 pb-24 bg-[#3F4143]">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <FadeIn>
            <Link href="/clients" className="inline-block text-white/50 text-xs font-bold tracking-[0.2em] mb-6 hover:text-[#FFD91A] transition-colors">
              ← FOR CLIENTS
            </Link>
      ...
      <section className="py-24 bg-[#3F4143]">
        <div className="max-w-5xl mx-auto px-6 lg:px-10 text-center">
          <FadeIn>
            ...
            <Link href="/contact" className="inline-block bg-[#FFD91A] text-[#3F4143] px-8 py-4 text-sm font-bold tracking-[0.15em] hover:bg-white transition-colors">
              START A SEARCH →
            </Link>
          </FadeIn>
        </div>
...
function Story({ story, index }) {
  return (
    <article id={story.id} className="scroll-mt-24 bg-white border border-[#3F4143]/10">
      <header className="border-b border-[#3F4143]/10 p-8 md:p-12">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <span className="text-[#FFD91A] text-xs font-black tracking-[0.2em]">0{index + 1}</span>
          <span className="bg-[#F7F7F5] text-[#3F4143] px-3 py-1 text-[11px] font-semibold tracking-[0.1em] uppercase">{story.market}</span>
          <span className="bg-[#F7F7F5] text-[#3F4143] px-3 py-1 text-[11px] font-semibold tracking-[0.1em] uppercase">{story.region}</span>
        </div>
        <h2 className="text-2xl md:text-4xl font-black text-[#3F4143] leading-tight tracking-tight max-w-4xl">
          {story.title}
        </h2>
        <p className="mt-5 text-lg text-[#3F4143]/70 leading-relaxed max-w-3xl">{story.summary}</p>
      </header>

      <StaggerContainer className="divide-y divide-[#3F4143]/10" stagger={0.06}>
          ...
          <StaggerItem key={stage.label}>
            <div className="grid md:grid-cols-[minmax(0,260px)_1fr] gap-3 md:gap-10 p-8 md:px-12 md:py-8">
              <div className="flex items-start gap-3">
                <span className="text-[#FFD91A] text-xs font-black tracking-[0.15em] pt-1">0{i + 1}</span>
                <h3 className="text-sm font-black tracking-[0.12em] uppercase text-[#3F4143]">{stage.label}</h3>
              </div>
              <p className="text-[#3F4143]/75 leading-relaxed">{stage.body}</p>
            </div>
          </StaggerItem>
        ))}
      </StaggerContainer>
