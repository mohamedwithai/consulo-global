    ...
    <footer className="bg-charcoal text-white pt-20 pb-8">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-white/10">
          <div>
            <div className="text-signal text-xs font-bold tracking-[0.2em] mb-4">MARKETS</div>
            <ul className="space-y-2">
              {MARKETS.map((m) => (
                <li key={m.id}>
                  <Link href={`/markets/${m.id}`} className="text-sm text-white/70 hover:text-signal transition-colors">
                    {m.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <div className="text-signal text-xs font-bold tracking-[0.2em] mb-4">CONSULO</div>
            <ul className="space-y-2">
              <li><Link href="/about" className="text-sm text-white/70 hover:text-signal transition-colors">About</Link></li>
              <li><Link href="/contact" className="text-sm text-white/70 hover:text-signal transition-colors">Contact</Link></li>
              <li><a href="https://www.linkedin.com" target="_blank" rel="noreferrer" className="text-sm text-white/70 hover:text-signal transition-colors">LinkedIn</a></li>
              <li><Link href="#" className="text-sm text-white/70 hover:text-signal transition-colors">Privacy Policy</Link></li>
              <li><Link href="#" className="text-sm text-white/70 hover:text-signal transition-colors">Cookie Policy</Link></li>
              <li><Link href="#" className="text-sm text-white/70 hover:text-signal transition-colors">Terms</Link></li>
            </ul>
          </div>

          <div>
            <div className="text-signal text-xs font-bold tracking-[0.2em] mb-4">REGIONS</div>
            <ul className="space-y-2">
              {REGIONS.map((r) => (
